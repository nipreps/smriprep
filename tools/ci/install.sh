...
# Basic import check
python -c 'import smriprep; print(smriprep.__version__)'

if [ "$CHECK_TYPE" == "skiptests" ]; then
    exit 0
fi

pip install $EXTRA_PIP_FLAGS "smriprep[$CHECK_TYPE]"

set +eux

echo Done installing smriprep
